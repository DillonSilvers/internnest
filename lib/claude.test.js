...
const { buildMatchPrompt, parseMatchResponse, buildUserContent, modelFor, MODELS } = require('./claude');
...
test('buildMatchPrompt surfaces enriched eligibility requirements', () => {
  const enriched = [{ ...candidates[0], gpa_min: 3.5, class_years: ['Junior'], sponsorship: 'no', paid: true, deadline: '2026-10-01' }];
  const { user } = buildMatchPrompt({ industry: 'Finance' }, enriched);
  assert.match(user, /min GPA 3\.5/);
  assert.match(user, /for Juniors/);
  assert.match(user, /no visa sponsorship/);
  assert.match(user, /apply by 2026-10-01/);
});

test('resume flag adds the resume instruction to the prompt', () => {
  const withR = buildMatchPrompt({ industry: 'Finance' }, candidates, true);
  const without = buildMatchPrompt({ industry: 'Finance' }, candidates, false);
  assert.match(withR.user, /RESUME: the student's resume is attached/);
  assert.doesNotMatch(without.user, /RESUME:/);
});

test('buildUserContent wraps the pdf as a document block ahead of the text', () => {
  assert.equal(buildUserContent('hello'), 'hello');
  const c = buildUserContent('hello', { data: 'QUJD' });
  assert.equal(c.length, 2);
  assert.equal(c[0].type, 'document');
  assert.equal(c[0].source.media_type, 'application/pdf');
  assert.equal(c[1].text, 'hello');
});
